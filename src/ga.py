...
class HamiltonianGA(BaseEstimator):
    ...
    def __init__(self, pop_size=100, n_gen=200, mutation_rate=0.02,
...
if __name__ == "__main__":
    from generate_graphs import GraphFactory
    G=GraphFactory('scale_free',n=20,m=8)()

    for (u, v) in G.edges():
        G.edges[u, v]['weight'] = random.randint(1, 10)
    ga = HamiltonianGA(pop_size=100, n_gen=100, mutation_rate=0.05,
                       fitness_mode='soft', penalty_weight=10, verbose=True, memoize=True)
    ga.fit(G)
    ga.is_viable(G)
    ...
    plot_population_distribution(ga.all_genomes_, lambda ind: f.soft_penalty(ind, nx.to_numpy_array(G), penalty_weight=10)).show()
